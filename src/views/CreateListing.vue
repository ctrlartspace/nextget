    ...
    <div class="row my-0 gx-2 gy-2">
      <div class="col-md-6 order-2 order-md-1">
        ...
        <div class="padding colored rounded-bottom section dashed">
          ...
          <div class="offset-s"></div>
          <div class="info black inline">
            <p>Комплектация: {{ selectData.equipment.value }}</p>
          </div>
        </div>
        ...
        <div class="padding colored rounded section">
          ...
          <div v-show="currentStep == 2">
            ...
            <div class="row gx-1 gy-1">
              <div class="col-12">
                <select v-model.trim="selectData.condition_state" :disabled="isProductSelected">
                  <option value="0" selected disabled>Состояние</option>
                  <option v-for='conditionState in getConditions' :value="{ id: conditionState.id, value: conditionState.value }" :key="conditionState.id">
          ...
          <div v-show="currentStep == 3">
            ...
            <div class="row gx-1 gy-1">
              <div class="col-12">
                <select v-model.trim="selectData.equipment" :disabled="isProductSelected">
                  <option value="0" selected disabled>Комплектация</option>
                  <option v-for='equipment in getEquipments' :value="{ id: equipment.id, value: equipment.value }" :key="equipment.id">
                    {{ equipment.value }}
                  </option>
                </select>
              </div>
          ...
          <div v-show="currentStep == 4">
            <div class="input-data">
              <input type="number" v-model="selectData.price"  placeholder="Цена" step="500" :disabled="isProductSelected">
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    createListing() {
      ...
      this.$store.dispatch('createListing', data).then(() => {
